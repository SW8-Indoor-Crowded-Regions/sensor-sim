...
def mock_rooms(mocker):
	...
	mock_room_data = [
		{'id': 1, 'name': 'Room 1', 'type': 'office', 'crowd_factor': 1.5, 'popularity_factor': 1.2, 'area': 50.0},
		{'id': 2, 'name': 'Room 2', 'type': 'conference', 'crowd_factor': 2.0, 'popularity_factor': 1.2, 'area': 100.0},
	]

	mock_model = mocker.patch(
		'app.utils.load_objects.RoomModel.objects', return_value=mock_room_data
	)
	return mock_model
...
def mock_sensors(mocker):
	...
	mock_model = mocker.patch(
		'app.utils.load_objects.SensorModel.objects', return_value=mock_sensor_data
	)
	return mock_model
