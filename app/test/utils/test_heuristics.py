...
def test_get_weights(visitor, rooms):
    ...
    assert all(isinstance(weight, (float, int)) for weight in get_weights(visitor.get_movement_options(), visitor))
    
    # Ensure the function returns a list of the same length as the movement options
    assert len(get_weights(visitor.get_movement_options(), visitor)) == len(visitor.get_movement_options())
